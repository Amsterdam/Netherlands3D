...
namespace Netherlands3D.ModelParsing
{
    public class ReadMTL : MonoBehaviour
    {
        ...
        public void StartMTLParse(string data, System.Action<bool> callback, string filename)
        {
            ...
            parseLinePointer = 0;
            materialDataSlots = new List<MaterialData>();
            StartCoroutine(ParseMTLFile(callback));
        }

        private IEnumerator ParseMTLFile(System.Action<bool> callback)
        {
            isBusy = true;
            ...
            while (remainingLinesToParse > 0)
            {
                ...
                float percentage = 100*(1.0f - (remainingLinesToParse / totalLines));
                if(broadcastProgressPercentage!=null) broadcastProgressPercentage(percentage);
        ...
        private int ParseNextMtlLines(int maxLines)
        {

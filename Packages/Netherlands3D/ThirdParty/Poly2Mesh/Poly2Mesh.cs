...
public static class Poly2Mesh
{
    ...
    public static Mesh CreateMesh(Polygon polygon, float thickness = 0, bool addBottom = false)
	{
        ...
        if (thickness > 0)
        {
            if (addBottom) AddBottom(thickness,vertexList, indices);
    ...
    private static void AddBottom(float height, List<Vector3> vertexList, List<int> indices)
    {
        //Copy current triangles, subtract height and add reversed indices
        var vertOffset = new Vector3(0, -height, 0);
        var bottomVertices = new List<Vector3>(vertexList);
        var bottomStartIndex = vertexList.Count;

        for (int i = 0; i < bottomVertices.Count; i++)
        {
            bottomVertices[i] = bottomVertices[i] + vertOffset;
        }
        ...
        {
            bottomIndices.Add(indices[i] + bottomStartIndex);
        }

        vertexList.AddRange(bottomVertices);

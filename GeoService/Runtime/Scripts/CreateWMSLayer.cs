...
namespace Netherlands3D.Geoservice
{
    public class CreateWMSLayer : MonoBehaviour
    {
        ...
        public UnityEngine.Experimental.Rendering.Universal.RenderObjects wmsStencil;
        ...
        public void ChangeProjectiontarget(int i)
        {
            if (wmsStencil != null)
            {
                foreach (var item in layer.tiles.Values)
                {
                    item.gameObject.GetComponent<MeshRenderer>().material.SetFloat("_StencilRef", i);
                    
                }
                //wmsStencil.settings.filterSettings.LayerMask = layermasks[i];
                //UnityEngine.Experimental.Rendering.Universal.RenderObjects.FilterSettings filtersettings = wmsStencil.settings.filterSettings;
                //filtersettings.LayerMask = layermasks[i];
                //wmsStencil.settings.filterSettings = filtersettings;
                //wmsStencil.Create();

...
public class ColladaFile
{
	...
	private Dictionary<string, List<Vector3>> objectsTriangles;

	private bool generateGeoHeader = false;
	private Vector3WGS geoCoordinates = default;

	public void AddObject(List<Vector3> triangleVertices, string objectName, Vector4 objectColor)
	{
		if (objectsTriangles == null) objectsTriangles = new Dictionary<string, List<Vector3>>();
		if (colors == null) colors = new List<Vector4>();
		if (materialnames == null) materialnames = new List<string>();

		if (!materialnames.Contains(objectName))
		{
			materialnames.Add(objectName);
	...
	public void CreateCollada(bool addGeoLocation = false, Vector3WGS geolocationWGS = default)
	{
		...
		int counter = 0;
		foreach (Vector4 material in colors)
		{
			WriteMaterialEffects(material,materialnames[counter]);
			counter++;
		}
		...
		foreach (Vector4 material in colors)
		{
			WriteMaterial(material,materialnames[counter]);
			counter++;
		}
		writer.WriteEndElement();
	...
	private void WriteGeometry(KeyValuePair<string, List<Vector3>> meshNameAndVerts, bool writeVertexNormals = false)
	{
		...
		for (int i = 0; i < meshVertices.Count; i++)
		{
			...
			writer.WriteString(vert.x + " " + vert.z + " " + vert.y);
	...
	private void WriteMaterial(Vector4 material, string objectName)
	{
		writer.WriteStartElement("material");
		writer.WriteAttributeString("id", objectName);
		writer.WriteStartElement("instance_effect");
		writer.WriteAttributeString("url", "#" + objectName + "-effect");
		writer.WriteEndElement();
		writer.WriteEndElement();
	}
	...
	private void WriteMaterialEffects(Vector4 materialColor, string objectName)
	{

		writer.WriteStartElement("effect");
		writer.WriteAttributeString("id", objectName + "-effect");
		...
		writer.WriteStartElement("color");
		writer.WriteString(materialColor.x + " " + materialColor.y + " " + materialColor.z + " " + materialColor.w);
		writer.WriteEndElement();
		writer.WriteEndElement();
	...
	{
#if UNITY_EDITOR
		var mydocs = Application.persistentDataPath;

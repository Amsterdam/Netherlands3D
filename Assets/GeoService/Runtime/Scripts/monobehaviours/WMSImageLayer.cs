...
namespace Netherlands3D.Geoservice
{
    public class WMSImageLayer : Layer
    {
        ...
        public void ProjectOnBuildings()
        {
            if (BuildingStencilID==0)
            {
                BuildingStencilID = LayerMask.NameToLayer("Buildings");
        ...
        private void setProjectorStencilSettings()
        {
            ...
            {
                tile.Value.gameObject.GetComponent<MeshRenderer>().material.SetFloat("_StencilRef", activeStencilID);
                tile.Value.gameObject.GetComponent<MeshRenderer>().material.SetFloat("_ReadMask", activeStencilMask);
            }
        }
